...
var _ = Describe("Operator for Cluster Mode", func() {
	...
	BeforeEach(func() {
		...
		Eventually(func() error {
			return checkKvrocksCluster(kvrocksInstance, sentinelInstance)
		}, timeout, interval).Should(Succeed())
	})
...
func checkKvrocksCluster(kvrocksInstance, sentinelInstance *kvrocksv1alpha1.KVRocks) error {
	slots := []int{}
	masterIP := []string{}
	password := kvrocksInstance.Spec.Password
	version := kvrocksInstance.Status.Version
	for _, partition := range kvrocksInstance.Status.Topo {
		for _, topo := range partition.Topology {
			...
			nodeVersion, err := kvrocksClient.ClusterVersion(topo.Ip, password)
			if err != nil {
				return err
			}
